...
-(void) getParcels{
    ...
    [conn getParcels:currentBeans.insurance.contract issuance:[issuance intValue] ciaCode:currentBeans.insurance.ciaCode];
...
//        if(payment.canExtend){
//            extends = [[ExtendParcelViewController alloc] initPaymentBeans:payment contract:currentBeans.insurance.contract issuance:[(NSNumber*)[currentBeans.insurance.issuances objectAtIndex:indexPayment] intValue] ciaCode:currentBeans.insurance.ciaCode ClientCode:currentBeans.insurance.cifCode];
//            [extends setDelegate:self];
//            [self presentViewController:extends animated:YES completion:nil];
//        }else if(payment.status == 0  && [[payment.codigoTipoModalidadeCobranca uppercaseString] isEqualToString:@"FB"]){
//            //uncoment it to enable get barcode number
////            [view showPopUpLoading];
////
////            [model getInstallmentPaymentLine:payment.number contract:currentBeans.insurance.contract issuance:[(NSNumber*)[currentBeans.insurance.issuances objectAtIndex:indexPayment] intValue]];
//        }
        
//        switch (payment.showComponent) {
//            case 1:
//                //Ticket
//                getDigitableLine = [[DigitableLineViewController alloc] initPaymentBeans:payment contract:currentBeans.insurance.contract issuance:[(NSNumber*)[currentBeans.insurance.issuances objectAtIndex:indexPayment] intValue] ciaCode:currentBeans.insurance.ciaCode ClientCode:currentBeans.insurance.cifCode];
...
//        }
        if(payment.showComponent > 0){
            paymentController = [[PaymentPopUpViewController alloc] initPaymentBeans:payment contract:currentBeans.insurance.contract issuance:[(NSNumber*)[currentBeans.insurance.issuances objectAtIndex:indexPayment] intValue] ciaCode:currentBeans.insurance.ciaCode  ClientCode:currentBeans.insurance.cifCode issuingAgency:currentBeans.insurance.issuingAgency];
