...
@implementation DADeletePolicyWS
...
- (void)deletePolicy:(DAPolicyBase *)policy {
	...
	NSString *soapMessage = [NSString stringWithFormat:							 
							 ...
							 "<soap:Body>\n"
							 "<DeletePolicy xmlns=\"http://tempuri.org/\">\n"
							 "<deviceUID>%@</deviceUID>\n"
	...
	//NSLog(@"DeletePolicy webServiceUrl:%@", [wsUrl absoluteString]);
	//NSLog(@"DeletePolicy soapMessage:\n%@", soapMessage);
	...
	[wsRequest addValue: @"http://tempuri.org/DeletePolicy" forHTTPHeaderField:@"SOAPAction"];
	...
	{
		//NSLog(@"DeletePolicy error: wsConnection is NULL");
		if ([delegate respondsToSelector:@selector(policyManager:deletePolicyDidFailWithErrorMessage:)])
			[delegate policyManager:nil deletePolicyDidFailWithErrorMessage:DALocalizedString(@"UnknownError", nil)];
	}	
}
...
{
	//NSLog(@"DeletePolicy error: %@", [error description]);
	
	if ([delegate respondsToSelector:@selector(policyManager:deletePolicyDidFailWithErrorMessage:)])
		[delegate policyManager:nil deletePolicyDidFailWithErrorMessage:[error localizedDescription]];
	
}

- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
	//NSLog(@"DeletePolicy Received Bytes: %d", [wsData length]);
	
	NSString *wsXML = [[NSString alloc] initWithBytes: [wsData mutableBytes] length:[wsData length] encoding:NSUTF8StringEncoding];
	//NSLog(@"DeletePolicy XML:\n%@", wsXML);
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
	if([elementName isEqualToString:@"ResultCode"] || [elementName isEqualToString:@"ErrorMessage"]) {
	...
	else if ([elementName isEqualToString:@"DeletePolicyResult"]) {
		recordEnabled = NO;
		
		//NSLog(@"DeletePolicy ResultCode: %@", [wsResults objectForKey:@"ResultCode"]);
		//NSLog(@"DeletePolicy ErrorMessage: %@", [wsResults objectForKey:@"ErrorMessage"]);
		...
		if (actionResult.resultType == kDAResultSuccess) {
			if ([delegate respondsToSelector:@selector(policyManager:didDeletePolicy:)])
				[delegate policyManager:nil didDeletePolicy:policyToDelete];
		}
		else {
			if ([delegate respondsToSelector:@selector(policyManager:deletePolicyDidFailWithErrorMessage:)])
				[delegate policyManager:nil deletePolicyDidFailWithErrorMessage:[actionResult errorMessage]];
		}		
